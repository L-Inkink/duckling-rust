...
fn _generate_instant_rules(b: &RuleSetBuilder<Value>, ctx: Arc<TimeContext>) {
    // ========================================
    // Instant patterns from Vietnamese
    // ========================================

    // "bây giờ", "hiện tại", "giờ" - now
    let ctx_now = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:now",
        b.reg(r"bây giờ|hiện tại|giờ").unwrap(),
    ...
    // "ngày mốt", "mai sau" - day after tomorrow
    let ctx_day_after = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:day_after_tomorrow",
        b.reg(r"ngày mốt|mai sau").unwrap(),
        move |_| {
    ...
    // "hôm kia", "ngày kia" - day before yesterday
    let ctx_day_before = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:day_before_yesterday",
        b.reg(r"hôm kia|ngày kia").unwrap(),
        move |_| {
...
fn _generate_dow_rules(b: &RuleSetBuilder<Value>, ctx: Arc<TimeContext>) {
    ...
    b.rule_1_terminal(
        "vi:time:dow:wednesday",
        b.reg(r"thứ\s*tư").unwrap(),
        move |_| {
    ...
    b.rule_1_terminal(
        "vi:time:dow:thursday",
        b.reg(r"thứ\s*năm").unwrap(),
        move |_| {
    ...
    b.rule_1_terminal(
        "vi:time:dow:saturday",
        b.reg(r"thứ\s*bảy").unwrap(),
        move |_| {
...
fn _generate_month_rules(b: &RuleSetBuilder<Value>, ctx: Arc<TimeContext>) {
    ...
    // Tháng một (January - month 1)
    let ctx_january = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:january",
        b.reg(r"tháng\s*một").unwrap(),
        move |_| {
    ...
    // Tháng hai (February - month 2)
    let ctx_february = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:february",
        b.reg(r"tháng\s*hai").unwrap(),
        move |_| {
    ...
    // Tháng ba (March - month 3)
    let ctx_march = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:march",
        b.reg(r"tháng\s*ba").unwrap(),
        move |_| {
    ...
    // Tháng tư (April - month 4)
    let ctx_april = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:april",
        b.reg(r"tháng\s*tư").unwrap(),
        move |_| {
    ...
    // Tháng năm (May - month 5)
    let ctx_may = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:may",
        b.reg(r"tháng\s*năm").unwrap(),
        move |_| {
    ...
    // Tháng sáu (June - month 6)
    let ctx_june = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:june",
        b.reg(r"tháng\s*sáu").unwrap(),
        move |_| {
    ...
    // Tháng bảy (July - month 7)
    let ctx_july = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:july",
        b.reg(r"tháng\s*bảy").unwrap(),
        move |_| {
    ...
    // Tháng tám (August - month 8)
    let ctx_august = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:august",
        b.reg(r"tháng\s*tám").unwrap(),
        move |_| {
    ...
    // Tháng chín (September - month 9)
    let ctx_september = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:september",
        b.reg(r"tháng\s*chín").unwrap(),
        move |_| {
    ...
    // Tháng mười (October - month 10)
    let ctx_october = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:october",
        b.reg(r"tháng\s*mười").unwrap(),
        move |_| {
    ...
    // Tháng mười một (November - month 11)
    let ctx_november = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:november",
        b.reg(r"tháng\s*mười\s*một").unwrap(),
        move |_| {
    ...
    // Tháng mười hai (December - month 12)
    let ctx_december = Arc::clone(&ctx);
    b.rule_1_terminal(
        "vi:time:month:december",
        b.reg(r"tháng\s*mười\s*hai").unwrap(),
        move |_| {
...
fn _generate_simple_rules(b: &RuleSetBuilder<Value>, ctx: Arc<TimeContext>) {
    ...
    // Examples: "buổi sáng", "buổi trưa", "buổi chiều", "buổi tối", etc.
}
